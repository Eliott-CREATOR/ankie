...
| `wordfreq==3.1.1` | one-off tool, `tools/frequency/export_wordfreq.py` — not a workspace dependency | Exports the English frequency ranking that `apps/worker/scripts/build-frequency-bands.mjs` turns into `data/frequency/en-bands.json` (C3 frequency bands). Run once by hand in a throwaway venv; never imported by the Worker or CI. Data is CC BY-SA 4.0 — see `data/frequency/LICENSE.md`. Pinned exactly so a re-export is reproducible. |
