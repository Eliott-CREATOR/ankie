# Ankie — standards for every agent

Full context: `docs/spec.md` (architecture) and `docs/roadmap.md` (build order, review
philosophy). Read both before starting a chantier. This is the canonical, tool-agnostic ruleset
every agent working on this project follows — Claude and Codex alike. It is not a replacement for
the spec or the roadmap, and it is not specific to any one tool: a tool-specific file (`CLAUDE.md`
for Claude Code today) covers only what's specific to running as that tool.
...
4. **Deterministic tools own formatting, linting, types and tests.** Biome, `tsc`, Vitest. No
   agent — subagent, hook, or otherwise — is to comment on anything those tools already check; a
   review pass reviews semantics only: correctness under edge cases, security, data loss, genuine
   design problems.
...
- C0 (the first chantier) contains **zero application logic** — no cards, no FSRS, no scheduling,
  no MCP tools. It builds the scaffolding and the review harness that gates every later chantier.
...
Every chantier gate needs a semantic review pass, on top of what deterministic tools already
check. Deterministic tools — Biome, `tsc`, Vitest — own formatting, linting, types and tests; no
agent reviews what those already check (rule 4, above). A semantic review pass looks only at what
a tool cannot decide: correctness under edge cases, security, data loss, and genuine design
problems.

How that pass is actually run — hooks, subagents, CI wiring, model choice — is specific to each
agent's tooling and lives in that agent's own file, not here. For Claude Code today, that's
`CLAUDE.md`'s "Review harness" section.

## Working with two agents

Two agents work this codebase, split by what each is good at:

- **Claude** — architecture, backend, database, migrations, API design, auth/security,
  spaced-repetition logic, hard debugging, technical decisions with long-term consequences,
  reviewing Codex's work.
- **Codex** — frontend implementation, UI/animations, responsive behavior, component work,
  isolated tests, small parallel-safe tasks.

**Never modify the same files at the same time.** Check `docs/agent-handoff.md` before starting
anything — it names what's currently active and who's touching what.

**Before handing work to the other agent, update `docs/agent-handoff.md`** with: objective,
current status, files changed, technical decisions, contracts, tests run, known issues, and the
exact next step. A handoff doc that's stale is worse than no handoff doc — it reads as current
when it isn't.

**Non-negotiable, no exception: never print a secret value into a report, commit message, log, or
test output.** Say "rotated" or "set" and stop. This project has already burned two passwords that
way.

## Engineering lessons

Principles generalized from real incidents on this project. The incidents themselves — with their
specific tool names and reproduction steps — are recorded where they're actually actionable: in
each agent's tool-specific debugging notes (`CLAUDE.md`'s "Debugging" section for Claude Code).
What belongs here is the shape of the trap, since it will recur in a different form.

**The materialized-snapshot trap.** Any denormalized copy of data — a JSON snapshot taken at
write time, a cached column, a precomputed field — goes stale the moment the source it was copied
from changes, unless the same write that changes the source also refreshes every copy. Nothing
errors when this happens: the database is right, the copy is wrong, and the only symptom is a
screen that quietly shows old content. The fix is structural, not a discipline to remember: one
function that knows how to materialize a copy from its source, called from every write path that
can invalidate one — never hand-rolled twice, and never patched in one call site while another
still does it the old way.

**Never trust a single piped or rewritten command's output for anything security-critical** — a
secret value, a byte-exact comparison, anything a checkpoint or a security decision depends on.
Shells, proxies, and formatting layers between you and the real output can silently summarize,
reformat, or truncate it with no error. Verify through a written file, or some other independent
channel, before treating what a pipe handed you as ground truth.
